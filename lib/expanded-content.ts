...
const TESTIMONIALS = [
  ...
  // ── 5 new testimonials (Phase 8 expansion) ───────────────────────────────────
  `"[syn: We were looking for a private couple cafe | We tried several venues in Vadodara] and [syn: nothing came close | no one offered true privacy until we found] ${V}. [syn: The glass house was like a little world of our own | The rooftop felt magical with the city lights below]. [syn: Our anniversary was simply perfect | One of the best nights of our lives]!" — [syn: Kunal] & [syn: Prachi], ${C}`,
  `"[syn: Booked the rooftop for my wife's birthday | Planned a candlelight surprise for our anniversary] and [syn: the team nailed every single detail | exceeded every expectation I had]. [syn: She cried happy tears | Her reaction was priceless] [syn: when she saw the setup | the moment she walked in]. [syn: Friends Factory Cafe is the real deal | 10 out of 10 experience]." — [syn: Manav] & [syn: Simran], ${C}`,
  `"[syn: The booking was so simple via WhatsApp | I was surprised how easy the reservation process was]. [syn: They confirmed everything immediately | Response was instant, very professional]. [syn: On the day, the setup was even more beautiful than the photos | Every detail was picture-perfect]. [syn: My boyfriend proposed and I said yes! | We're now engaged and it's all thanks to this magical setup]." — [syn: Nidhi] & [syn: Parth], ${C}`,
  `"[syn: We've celebrated three occasions here | This was our second time at Friends Factory Cafe] and [syn: the quality gets better every time | they surprised us with even more detail this visit]. [syn: The Rose Day setup they did | The Valentine's theme they created] was [syn: absolutely stunning | breathtakingly beautiful]. [syn: This is our celebration venue forever | We won't go anywhere else for our special days]." — [syn: Harsh] & [syn: Ruchi], ${C}`,
  `"[syn: I booked this as a surprise date for my girlfriend | I wanted to surprise my wife on our anniversary] and [syn: I was a little nervous | I wasn't sure how it would go]. [syn: But the staff at Friends Factory helped with every step | The team guided me through everything on WhatsApp]. [syn: She was completely stunned | The look on her face said it all]. [syn: Absolutely incredible value for money | Best ₹5,000 I've ever spent]." — [syn: Vihaan] & [syn: Kavya], ${C}`
];
...
const CLOSING_CTAS = [
  ...
  // ── 3 new closing CTAs (Phase 8 expansion) ───────────────────────────────────
  `[syn: Every slot fills quickly | Our most popular slots book out days in advance]. [syn: If your ideal date and time are important | If a particular slot matters for your plan], [syn: check availability now | don't wait — verify] by WhatsApp at ${PH}. [syn: Your perfect evening is only one message away | Your celebration starts with one text]!`,
  `[syn: Surprise your partner | Make your loved one feel truly special] with [syn: a completely private rooftop experience | an exclusive Glass House celebration] at ${V}. [syn: Starting at just ${LOW} | Packages from ${LOW} to ${HIGH}] — [syn: the most romantic evening in ${C} | an unforgettable private dinner] is within reach. WhatsApp ${PH} [syn: now | today] to [syn: check what's available | find the perfect date]!`,
  `[syn: The difference between a forgettable evening and a memory-for-life | What separates an ordinary dinner from a magical celebration] is [syn: the setting, the privacy, and the details | the thought, the space, and the execution]. At ${V}, [syn: we provide all three at an honest price | all three come together starting at ${LOW}]. [syn: Take the first step | Start planning now] — WhatsApp ${PH}.`
];
...
const OPENINGS = [
  ...
  // ── New 12 ───────────────────────────────────────────────────────────────────
  (kw: string) => `There is one thing every ${kw} in ${C} must have: complete privacy. At ${V}, your entire rooftop or glass house is exclusively reserved for you and your partner. No other tables, no strangers — just the two of you and a stunning hand-crafted setup. All-inclusive packages from ${LOW}.`,
  (kw: string) => `When it comes to ${kw} in ${C}, couples consistently choose ${V} for three reasons: unmatched privacy, breathtaking decoration, and stress-free booking. Our team handles everything — you simply arrive and celebrate.`,
  (kw: string) => `Not every venue that promises a romantic experience actually delivers one. ${V} is different. Our ${kw} in ${C} is built around 100% private spaces, fresh food, curated music, and a team that genuinely cares about your special moment. Packages from ${LOW}.`,
  (kw: string) => `The secret to a perfect ${kw} in ${C} is in the details — the soft music playing as you enter, the candles lit just right, the rose petals on the path. At ${V}, we obsess over every detail so your celebration is flawless from first minute to last.`,
  (kw: string) => `Couples searching for a truly private ${kw} in ${C} will find exactly what they need at ${V}. Unlike open restaurants where strangers can see you, our rooftop and glass house are 100% yours for the full 3-hour slot. Prices start at just ${LOW}.`,
  (kw: string) => `What does the ideal ${kw} in ${C} look like? At ${V}, it starts with you stepping into a beautifully decorated private space — fairy lights, flowers, your names on the letter board — and your partner's jaw dropping. 3,000+ couples have experienced this magic. Packages from ${LOW}.`,
  (kw: string) => `Choosing the right venue for ${kw} in ${C} is stressful. ${V} makes it easy. We offer 8 distinct packages to suit every budget from ${LOW} to ${HIGH}, coordinate every detail via WhatsApp, and guarantee a celebration your partner will talk about for years.`,
  (kw: string) => `If you want the best ${kw} experience in ${C}, look no further. ${V} combines a stunning 4th-floor rooftop view, a glass-enclosed private pavilion, expert decoration teams, and all-inclusive food and mocktails — at a price that won't break the bank.`,
  (kw: string) => `${V} has become Vadodara's most talked-about spot for ${kw} because we do something no other venue does: we make your celebration 100% private. Your space, your music, your moment. No interruptions. No compromise. Packages starting from ${LOW}.`,
  (kw: string) => `Planning ${kw} in ${C} doesn't have to be complicated. At ${V}, all it takes is one WhatsApp message to ${PH}. Our team handles decorations, food, music, and timing while you focus on enjoying the most romantic evening of your life. All-inclusive from ${LOW}.`,
  (kw: string) => `The best ${kw} in ${C} starts with a venue that truly understands romance. At ${V}, our team has hosted over 3,000 love celebrations — birthdays, anniversaries, proposals, and intimate dinners. That experience shows in every lit candle, every flower arrangement, and every perfectly timed surprise.`,
  (kw: string) => `Imagine walking into a space built entirely for your ${kw} in ${C} — fairy lights twinkling, your favorite songs playing, rose petals on the floor, and your partner's eyes lighting up. That's exactly what ${V} delivers, starting from just ${LOW} per couple. Book via WhatsApp: ${PH}.`,
];
